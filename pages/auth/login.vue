  ...
  <div class="min-h-screen bg-[#fef6f7] flex items-center justify-center p-6">
    <div class="w-full max-w-sm">
      ...
      <div class="bg-white rounded-xl p-6 shadow-sm">
        <div class="flex items-center bg-gray-50 rounded-lg border border-gray-200 focus-within:border-red-300 transition-all mb-5">
        ...
        <div v-if="codeSent" class="space-y-4 animate-fade-in">
          ...
          <button type="button"
            ...
            :disabled="loading || inputCode.length !== 6" @click="doLogin">
            {{ loading ? '登录中...' : '验证并登录' }}
          </button>
        </div>
      </div>
...
async function doLogin() {
  ...
  try {
    ...
    // 2. If new user, register via API (handles email confirm)
    if (error) {
    ...
    { window.$toast('登录失败: ' + (e.message || '未知错误'), 'error'); }
  }
  loading.value = false
}
</script>
