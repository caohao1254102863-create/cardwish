...
import { u as useI18n, a as useLocalePath, f as useRouter } from './server.mjs';
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        ssrRenderList(unref(cards), (card) => {
          ...
          _push(`<div class="${ssrRenderClass([getGradient(card.category_slug), "aspect-[3/4] flex items-center justify-center text-5xl"])}"> \u{1F49D} </div><div class="p-3"><p class="text-xs text-gray-400">${ssrInterpolate(unref(t)("received.from", { payer: card.payer_nickname || "A friend" }))}</p><p class="text-xs text-gray-300">${ssrInterpolate(formatDate(card.created_at))}</p></div></div>`);
        });
        _push(`<!--]--></div>`);
      }
      _push(`</div></div>`);
...
//# sourceMappingURL=received-BRpCamKv.mjs.map
