...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const currencySymbol = computed(() => {
      var _a;
      if (((_a = card.value) == null ? void 0 : _a.currency) === "CNY") return "\xA5";
      return "$";
    });
    ...
    const categoryEmoji = computed(() => {
      ...
      return map[((_a = card.value) == null ? void 0 : _a.category_slug) || ""] || "\u{1F49D}";
    });
    function formatPrice(cents) {
      return (cents / 100).toFixed(2);
    }
    useHead(() => ({
      title: card.value ? `${card.value.name} - CardWish` : "Card Details - CardWish"
    }));
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        _push(`<div class="p-4"><div class="aspect-[3/4] bg-gray-100 rounded-xl animate-shimmer mb-4"></div><div class="h-6 bg-gray-100 rounded w-3/4 animate-shimmer mb-2"></div><div class="h-4 bg-gray-100 rounded w-1/2 animate-shimmer"></div></div>`);
      } else if (unref(error)) {
        ...
        if ((_b = (_a = unref(card).images) == null ? void 0 : _a[0]) == null ? void 0 : _b.url) {
          _push(`<img${ssrRenderAttr("src", unref(card).images[0].url)}${ssrRenderAttr("alt", unref(card).name)} class="w-full aspect-[4/3] object-cover">`);
        } else {
          _push(`<div class="${ssrRenderClass([unref(gradientClass), "w-full aspect-[4/3] flex items-center justify-center text-7xl"])}">${ssrInterpolate(unref(categoryEmoji))}</div>`);
        }
        _push(`</div><div class="px-4 pt-5"><div class="flex items-start justify-between mb-2"><h1 class="text-2xl font-bold text-gray-800 flex-1">${ssrInterpolate(unref(card).name)}</h1></div><p class="text-gray-500 text-sm mb-4">${ssrInterpolate(unref(card).description)}</p><div class="flex items-center gap-2 mb-4"><span class="badge bg-pink-50 text-[var(--color-primary)]">${ssrInterpolate(unref(card).category_name)}</span><!--[-->`);
        ssrRenderList(unref(card).tags, (tag) => {
          _push(`<span class="badge bg-gray-100 text-gray-500">${ssrInterpolate(tag)}</span>`);
        });
        _push(`<!--]--></div><div class="card-container p-4 mb-6"><div class="flex items-center justify-between"><span class="text-gray-500">${ssrInterpolate(unref(t)("card.price"))}</span><span class="price-tag text-2xl"><span class="text-sm mr-0.5">${ssrInterpolate(unref(currencySymbol))}</span> ${ssrInterpolate(formatPrice(unref(card).price_cents))}</span></div></div>`);
        _push(ssrRenderComponent(_component_NuxtLink, {
...
//# sourceMappingURL=_id_-B9aD-vQs.mjs.map
