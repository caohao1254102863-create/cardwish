...
const captureAndComplete_post = defineEventHandler(async (event) => {
  try {
    ...
    const { data: order } = await supabase.from("orders").select("id, creator_id, total_cents, currency, status").eq("share_code", share_code).single();
    if (!order) {
    ...
    const { data: payment } = await supabase.from("payments").select("id, paypal_order_id").eq("order_id", order.id).eq("status", "pending").eq("payment_method", "paypal").order("created_at", { ascending: false }).limit(1).single();
    if (payment) {
...
export { captureAndComplete_post as default };
//# sourceMappingURL=capture-and-complete.post.mjs.map
